...
import com.dlb.exception.ErrorCode;
import com.dlb.exception.FastDFSException;
import com.dlb.utils.fastdfs.server.TrackerServerPool;
...
public class FastDfsClient {
     ...
     * FastDFS秘钥
     */
    public static String fastDFSHttpSecretKey=null;
    /**
     * 路径分隔符
     */
     ...
     * org.slf4j.Logger
     */
    private static org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(FastDfsClient.class);
    /**
     * 文件名称Key
     */
    ...
    public static String getToken(String filepath){
        ...
        String token = "null";
        try {
            token = ProtoCommon.getToken(getFilename(filepath), ts, fastDFSHttpSecretKey);//秘钥
     ...
     * @return 返回字节
     */
    public byte[] downloadFile(String groupName,String filename) throws FastDFSException {
        TrackerServer trackerServer = TrackerServerPool.borrowObject();
        try {
    ...
    public void download(String filepath, String filename, OutputStream os, HttpServletResponse response) throws FastDFSException {
        ...
        //把路径统一格式=====group1/M00/00/00/L2WlYGHX32CAT_c9ADHmse0OfXM266.mp4
        filepath = toLocal(filepath);
        // 获取文件名
        if (StringUtils.isBlank(filename)) {//如果文件名为空
            filename = getOriginalFilename(filepath);//根据路径获取文件名
        }
        // 文件后缀====.MP4
     ...
     */
    public int deleteFile(String groupName,String filename) throws FastDFSException {
        TrackerServer trackerServer = TrackerServerPool.borrowObject();
        try{
            StorageClient storageClient = new StorageClient(trackerServer);
